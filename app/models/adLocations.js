const MainModel 	= require(__path_schemas + 'adLocations');
...
module.exports = {
    listItems: (params, options = null) => {
        ...
        return MainModel
            .find(objWhere)
            .select('name status link ordering created modified slug')
            .sort(sort)
    ...
    getItems: (params = null, option = null) => {
        if(option.task == 'get-items-by-id'){
            return MainModel.findById(params.id).select('name slug link');
        }
        if(option.task == 'get-name-items'){
            return MainModel.find({}, {_id: 1, name: 1, link: 1});
        }
        if(option.task == 'get-items-by-slug'){
            return MainModel.find({slug: params.slug}).select('name slug link');
        }
    },

    listItemsInSelectbox: (params, options = null) => {
        return  MainModel.find({}, {_id: 1, name: 1, slug: 1, link: 1});
    },
